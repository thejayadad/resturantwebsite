...
export default async function ItemDetailPage({ params }: Params) {
  ...
  // Load attached groups + their options
  ...
  return (
    <div className="mx-auto max-w-3xl p-6 space-y-6">
      <div className="flex items-center justify-between">
        <Link href="/menu" className="text-sm underline">← Back to Menu</Link>
      </div>

      <div className="space-y-1">
        <h1 className="text-2xl font-bold">
          {item.title}{' '}
          {item.code ? <span className="text-sm text-neutral-500">({item.code})</span> : null}
        </h1>
        {item.description ? <p className="text-neutral-600">{item.description}</p> : null}
        <p className="text-sm text-neutral-500">
          From <span className="font-medium">{defaultPrice}</span>
        </p>
      </div>
      ...
      <section className="space-y-2">
        ...
        <div className="flex flex-wrap gap-2">
          {item.variants.map(v => (
            <span
              key={v.id}
              className={`rounded-full border px-3 py-1 text-sm ${v.isDefault ? 'bg-green-50 border-green-200' : ''}`}
            >
              {v.name}: {formatUSD(v.price.toString())}{v.isDefault ? ' • default' : ''}
            </span>
          ))}
        </div>
      </section>
      ...
      <section className="space-y-4">
        ...
        {attachments.map(att => {
          ...
          // Effective rules (item overrides group defaults)
      ...
      <div className="border-t pt-4">
        <button className="rounded-lg bg-neutral-900 text-white px-4 py-2 cursor-not-allowed opacity-60">
